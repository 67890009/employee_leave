...
class LeaveRequestService:
    ...
    async def apply_leave(self, db: AsyncSession, current_user: User, data: LeaveRequestCreate) -> LeaveRequest:
        overlapping_leave = await self.repository.get_overlapping_leave(
            db, current_user.id, data.start_date, data.end_date,
        )
        if overlapping_leave:
            raise HTTPException(status_code=400, detail="Leave request overlaps with an existing leave.")
        ...
        if not leave_type.is_active:
            raise HTTPException(status_code=400, detail="This leave type is no longer active.")
        ...
        if data.number_of_days > leave_type.max_days_per_year:
            raise HTTPException(status_code=400, detail=f"Maximum allowed is {leave_type.max_days_per_year} days.")
        ...
        if data.number_of_days > remaining_days:
            raise HTTPException(status_code=400, detail=f"Only {remaining_days} leave days remaining.")
    ...
    async def get_leave_request(self, db: AsyncSession, leave_request_id: UUID, current_user: User) -> LeaveRequest:
    ...
    async def cancel_leave_request(self, db: AsyncSession, leave_request_id: UUID, current_user: User) -> LeaveRequest:
        leave_request = await self.repository.get_by_id(db, leave_request_id)
        if leave_request is None:
            raise HTTPException(status_code=404, detail="Leave request not found.")
    ...
    async def approve_leave(self, db, leave_request_id, current_user, data):
        return await self._process_approval(db, leave_request_id, current_user, data, LeaveStatus.APPROVED)

    async def reject_leave(self, db, leave_request_id, current_user, data):
    ...
    async def _process_approval(
        ...
        data: LeaveApprovalRequest, new_status: LeaveStatus,
    ) -> LeaveRequest:
        leave_request = await self.repository.get_by_id(db, leave_request_id)
        if leave_request is None:
            raise HTTPException(status_code=404, detail="Leave request not found.")
        ...
        if leave_request.status != LeaveStatus.PENDING:
            raise HTTPException(status_code=400, detail="Only pending leave requests can be processed.")
        ...
        if target_role == UserRole.EMPLOYEE:
            if current_user.role != UserRole.MANAGER or leave_request.employee.manager_id != current_user.id:
                raise HTTPException(status_code=403, detail="Only this employee's manager can approve or reject their leave.")
        elif target_role == UserRole.MANAGER:
            if current_user.role != UserRole.ADMIN:
                raise HTTPException(status_code=403, detail="A manager's leave request can only be approved by an Admin.")
        else:
            raise HTTPException(status_code=400, detail="This user's role does not support leave approval.")
